...
### 유틸리티 기능
- **데이터 복사** : 일반 파일/폴더 복사
- **공간 확인** : 소스/대상 드라이브 공간 확인 → 데이터 백업/복원, 데이터 복사(Copy Data)
- **장치 관리자 열기**, **파일 탐색기 열기**
...
3. **주요 버튼**
   - `데이터 백업(Backup Data)` → 사용자 데이터 전체 백업 → 브라우저(Chrome/Edge) 북마크 포함
   - `데이터 복원(Restore Data)` → 선택한 백업 폴더에서 백업 데이터 복원
   - `드라이버 백업(Backup Drivers)` / `드라이버 복원(Restore Drivers)` → 드라이버 관리
   - `앱 보내기(Export Apps)` → 설치 프로그램 목록 내보내기
   - `브라우저(Browser)` → Chrome/Edge/Firefox 프로필 백업
   - `예약(Schedule)` → 예약 백업 설정
   - `공간 확인(Check Space)` → 대상 드라이브 여유 공간 확인
