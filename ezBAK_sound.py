...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        version_label = tk.Label(title_container, text="v0.7.0", font=("Arial", 9), 
                                bg="#1a1f35", fg="#888888")
        ...
        main_header.pack(fill="x", padx=10, pady=(5, 0))
        ...
        self.sound_toggle_btn.pack(side="right")
    ...
    def _toggle_sound(self):
        """Sound on/off toggle"""
        try:
            current = self.sound_enabled_var.get()
            new_state = not current
            self.sound_enabled_var.set(new_state)
            
            if new_state:
                self.sound_toggle_btn.config(text="🔊 Sound ON", bg="#2E7D32")
                self.message_queue.put(('log', "Sound enabled"))
                # Test sound playback
                self._play_sound('complete')
            else:
                self.sound_toggle_btn.config(text="🔇 Sound OFF", bg="#7D98A1")
