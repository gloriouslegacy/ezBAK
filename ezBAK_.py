...
def log_message(message):
    """콘솔과 날짜별 로그파일(ezBAK_YYYY-MM-DD.log)에 동시 기록"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    log_filename = f"ezBAK_{datetime.now().strftime('%Y-%m-%d')}.log"
...
def _cleanup_old_logs(log_folder, retention_days):
    ...
    log_message(f"\n[Old log cleanup in '{log_folder}']")
    log_message(f"Scanning for log files older than {retention_days} days...")
        ...
        if filename.endswith(".log"):
            ...
            try:
                if os.path.isfile(filepath) and os.path.getmtime(filepath) < cutoff:
                    os.remove(filepath)
                    log_message(f"  - Deleting old log: {filename}")
            except Exception as e:
                log_message(f"  - ERROR: Could not process or delete log file {filename}: {e}")
...
class HeadlessBackupRunner:
    ...
    def run_backup(self):
        ...
        files_processed = 0
        user_name = self.user
        backup_path = self.dest
        user_profile_path = os.path.join("C:", os.sep, "Users", user_name)

        if not os.path.exists(user_profile_path):
            log_message(f"ERROR: User folder '{user_profile_path}' not found.")
            return

        timestamp = datetime.now().strftime("%Y-%m-%d")
        destination_dir = os.path.join(backup_path, f"{user_name}_backup_{timestamp}")

        log_message(f"Backup for user: {user_name}")
        log_message(f"Destination: {destination_dir}")
        log_message(f"Policy: Hidden={self.hidden_mode}, System={self.system_mode}, Keep={self.retention_count}")
        log_message("-" * 50)

        if os.path.exists(destination_dir):
            try: shutil.rmtree(destination_dir)
            except Exception as e: log_message(f"WARN: Failed to delete existing backup folder '{destination_dir}': {e}")

        os.makedirs(destination_dir, exist_ok=True)

        total_bytes = self.compute_total_bytes(user_profile_path)
        log_message(f"Calculating total size... {format_bytes(total_bytes)}")

        try:
            free = shutil.disk_usage(backup_path).free
            if total_bytes > free:
                log_message(f"\nERROR: Insufficient free space. Required: {format_bytes(total_bytes)}, Available: {format_bytes(free)}")
                return
        except Exception as e:
            log_message(f"WARN: Could not check disk space: {e}")

        log_message("\n[Backed up items list]")
        def progress_cb(delta): nonlocal bytes_copied; bytes_copied += delta

        for dirpath, dirnames, filenames in os.walk(user_profile_path, topdown=True, onerror=lambda e: log_message(f"WARN: os.walk error: {e}")):
            dirnames[:] = [d for d in dirnames if not self.is_hidden(os.path.join(dirpath, d))]
            ...
            for f in filenames:
                ...
                if self.is_hidden(src_file): continue
                log_message(os.path.relpath(src_file, user_profile_path))
                dest_file = os.path.join(dest_dir, f)
                self.copy_file_with_progress(src_file, dest_file, progress_cb)
                files_processed += 1

        log_message("-" * 50)
        log_message(f"Backup completed. Files backed up: {files_processed}, Total size: {format_bytes(bytes_copied)}")
        log_message("-" * 50)

        self.backup_browser_bookmarks(destination_dir)
        
        self._cleanup_old_backups(backup_path, user_name)
        _cleanup_old_logs(backup_path, self.log_retention_days)

        show_notification(
            title="ezBAK Backup Complete",
            message=f"Backup for '{user_name}' finished successfully.\n{files_processed} files backed up ({format_bytes(bytes_copied)}).",
            button_text="Open Folder",
            button_argument=destination_dir
        )
...
class App(tk.Tk):
    ...
    # Defaults remain: hidden/system = 'exclude' (set in __init__).
        ...
        # Github link label
        self.github_label = tk.Label(main_frame, text="https://github.com/gloriouslegacy", font=("Arial", 10, "underline"), fg="#2196F3", bg="#2D3250", cursor="hand2")
    ...
    def _cleanup_old_logs(self, log_folder, retention_days):
        """
        Deletes log files older than retention_days in the specified folder.
        This version parses the date from the filename, which is more reliable
        than file modification time if files are moved or copied.
        """
        ...
        if retention_days <= 0:
            ...
            self.write_detailed_log("Log cleanup is disabled (retention is 0 days or less).")
            return

        # Use date objects for comparison to ignore the time part.
        today = datetime.now().date()
        cutoff_date = today - datetime.timedelta(days=retention_days)

        self.write_detailed_log(f"\n[Old log cleanup in '{log_folder}']")
        self.write_detailed_log(f"Scanning for log files with a date older than {cutoff_date.strftime('%Y-%m-%d')}...")
        ...
        for filename in os.listdir(log_folder):
            ...
            # Match filenames like '..._2025-09-01_...log' or '..._20250901_...log'
            ...
            try:
                ...
                log_date = datetime.strptime(date_str, '%Y-%m-%d' if '-' in date_str else '%Y%m%d').date()
                
                if log_date < cutoff_date:
                    filepath = os.path.join(log_folder, filename)
                    if os.path.isfile(filepath):
                        os.remove(filepath)
                        self.write_detailed_log(f"  - Deleting old log: {filename}")
                        deleted_count += 1
            except (ValueError, IndexError):
                # Filename contains something that looks like a date but isn't, ignore.
                continue
            except Exception as e:
                self.write_detailed_log(f"  - ERROR: Could not process or delete log file {filename}: {e}")
        
        if deleted_count > 0:
            self.write_detailed_log(f"Cleanup finished. Deleted {deleted_count} old log file(s).")
        else:
            self.write_detailed_log("No old log files to clean up.")

    # Hidden/System options removed from UI as requested; defaults remain 'exclude'

    def open_github_link(self, event):
        webbrowser.open_new("https://github.com/gloriouslegacy")
    ...
    def open_log_file(self, base_folder, prefix):
        ...
        try:
            ...
            with self.log_lock:
                ...
                _cleanup_old_logs(base_folder, retention_days)
    ...
    def start_restore_thread(self):
        ...
        self.open_log_file(backup_folder, f"restore_{self.user_var.get()}")
    ...
    def run_restore(self, backup_folder):
        ...
        try:
            ...
            if not backup_folders:
                self.message_queue.put(('show_error', f"Backup folder for '{user_name}' not found."))
            ...
            # Free space pre-check (restore destination)
            try:
                free = shutil.disk_usage(destination_dir).free
            except Exception:
                free = self.get_free_space(destination_dir)
            if total_bytes > free:
                self.write_detailed_log(f"Insufficient free space for restore. Required={total_bytes} Free={free}")
                self.message_queue.put(('show_error', f"Not enough free space to restore.\nRequired: {self.format_bytes(total_bytes)}\nAvailable: {self.format_bytes(free)}"))
                return
            ...
            self.message_queue.put(('update_progress', bytes_copied))
            self.write_detailed_log("User folder restore completed.")
...
class ScheduleBackupDialog(tk.Toplevel):
    def __init__(self, master):
        ...
        # User
        # add_label("User")
        # self.user_var2 = tk.StringVar(value=default_user)
        # self.user_combo2 = ttk.Combobox(frm, textvariable=self.user_var2, values=users, state='readonly', width=32)
        # self.user_combo2.grid(row=row, column=1, sticky='w', pady=4)
        # row += 1
...
if __name__ == "__main__":
    ...
    def _run_headless_if_requested():
        """If --auto-backup is passed, run the backup and exit, with logging."""
        if '--auto-backup' not in sys.argv:
            return
        ...
        if not user or not dest:
            print("Missing --user or --dest for --auto-backup", file=sys.stderr)
            sys.exit(2)
        ...
        exit_code = 0
        try:
            # Ensure destination directory exists before trying to create a log file in it.
            os.makedirs(dest, exist_ok=True)
            log_file_path = os.path.join(dest, f"{user}_backup_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.log")
            # Use line buffering (buffering=1) to ensure writes are flushed more frequently.
            log_file_handle = open(log_file_path, 'w', encoding='utf-8', buffering=1)
            sys.stdout = log_file_handle
            sys.stderr = log_file_handle

            print(f"--- Headless execution started at {datetime.now()} ---")
            print(f"--- Arguments: {sys.argv} ---")
            sys.stdout.flush() # Explicitly flush after initial prints
            sys.stderr.flush()

            app = App()
            app.withdraw()
            
            app.message_queue = HeadlessLoggerQueue()
            app._log_file = log_file_handle
            app.log_file_path = log_file_path

            # Clean up old logs in headless mode
            try:
                retention_days = int(app.log_retention_days_var.get())
            except (ValueError, TypeError):
                retention_days = 30 # Fallback
            app._cleanup_old_logs(dest, retention_days=retention_days)

            app.hidden_mode_var.set('include' if include_hidden else 'exclude')
            app.system_mode_var.set('include' if include_system else 'exclude')
            ...
            exit_code = 1
            show_notification(
                title="ezBAK Backup Failed",
                message=f"Backup for user '{user}' encountered a fatal error. Check logs for details.",
                button_text="Open Log Folder",
                button_argument=dest
            )
        finally:
            if log_file_handle:
                print(f"--- Headless execution finished at {datetime.now()} ---")
                sys.stdout.flush() # Final flush before closing
                sys.stderr.flush()
                log_message(f"--- Headless execution finished at {datetime.now()} ---")
